const ORCH_BASE_URL = import.meta.env.VITE_ORCH_URL || 'http://localhost:9090'
const AUTHN_BASE_URL = import.meta.env.VITE_AUTHN_URL || 'http://localhost:9001/api'
...
export const orchAPI = {
  ...
  getTask: async (taskId: number): Promise<APIResponse<{ task: Task }>> => {
    return fetchAPI<{ task: Task }>(`${ORCH_BASE_URL}/v1/client/tasks/${taskId}`)
  },
  ...
  getTaskLogs: async (taskId: number): Promise<APIResponse<GetLogsResponse>> => {
  ...
  getAllPlugins: async (): Promise<APIResponse<{ plugins: Plugin[] }>> => {
    return fetchAPI<{ plugins: Plugin[] }>(`${ORCH_BASE_URL}/v1/client/plugins`)
  },
  ...
  createPlugin: async (
    ...
    return fetchAPI<{ plugin: Plugin }>(`${ORCH_BASE_URL}/v1/client/plugins/publish`, {
      method: 'POST',
      body: JSON.stringify(req),
    })
  },
